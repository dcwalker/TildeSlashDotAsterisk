...
## Deployment guidelines (follow unless the user opts out)

Personal skills and how they are stored, synced, and verified must follow **[Agent Skills Setup with Chezmoi](references/skills-setup.md)**. That reference file is the source of truth for:

- Single source of truth in chezmoi (`.skills-*.md.tmpl`, `.scripts-*.tmpl`, and `dot_cursor` / `dot_claude` / `dot_codex` template references), then `chezmoi apply`
- Deployed layout under `~/.cursor/skills/`, `~/.claude/skills/`, and `~/.codex/skills/` (identical copies across tools)
- Optional `scripts/`, `references/`, and `assets/` next to `SKILL.md`
- Script templates named with the `executable_` prefix under `scripts/` when using chezmoi, so deployed scripts are executable
- Editing in chezmoi source first (recommended), or syncing edits from `~/.cursor/skills/...` back into the shared template per Option 2 in that guide
- Verifying consistency (for example `md5` across the three deployed `SKILL.md` paths) after changes

If the user states that a skill is **project-only** (repository `.cursor/skills/`) or not managed with chezmoi, skip the chezmoi-specific steps and use **Storage Locations** below.
...
Skills are stored as directories containing a `SKILL.md` file. Align with the **Deployed Skills** layout in [references/skills-setup.md](references/skills-setup.md):

```
skill-name/
├── SKILL.md                 # Required - main instructions
├── scripts/                 # Optional - utility scripts (see [references/skills-setup.md](references/skills-setup.md) for chezmoi executable_ naming)
├── references/              # Optional - detailed docs (e.g. REFERENCE.md, examples.md)
└── assets/                  # Optional - templates, config snippets, etc.
```

Flat optional files next to `SKILL.md` (for example `reference.md`) are acceptable for small skills; prefer `references/` when adding multiple supporting files so layout matches the chezmoi deployment model.

### Storage Locations

| Type | Path | Scope |
|------|------|-------|
| Personal | ~/.cursor/skills/skill-name/ | Available across all your projects (when using chezmoi, this path is deployed from source per [references/skills-setup.md](references/skills-setup.md)) |
...
**Keep references one level deep** - link directly from SKILL.md to files under `references/` or other immediate children. Deeply nested references may result in partial reads.
...
1. Create the directory structure (`SKILL.md`, and optionally `scripts/`, `references/`, `assets/` per [references/skills-setup.md](references/skills-setup.md))
2. Write the SKILL.md file with frontmatter
3. Create any supporting files under `references/` or top-level as appropriate
4. Create any utility scripts under `scripts/` if needed (use chezmoi `executable_` script templates when that setup applies)
5. For personal chezmoi-managed skills: add or update the `.skills-*.md.tmpl` source and `dot_*` includes, run `chezmoi apply`, and verify checksums across tools as in that guide
...
### If Personal Skill (chezmoi / multi-tool)
- [ ] Changes follow [references/skills-setup.md](references/skills-setup.md) (source template, `dot_cursor` / `dot_claude` / `dot_codex`, `chezmoi apply`, verification)
- [ ] Layout includes `references/` or `assets/` when the skill has non-trivial supporting files
