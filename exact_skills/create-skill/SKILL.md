...
- Single source of truth in `exact_skills/<name>/` within the chezmoi source directory
- Symlinks from `~/.cursor/skills/`, `~/.claude/skills/`, and `~/.codex/skills/` all point to `~/skills/`, so one copy serves all three tools
- Optional `scripts/`, `references/`, and `assets/` next to `SKILL.md`
- Script files named with the `executable_` prefix under `scripts/` in chezmoi source, so deployed scripts are executable
- Edit skills directly in the chezmoi source `exact_skills/<name>/` directory, then `chezmoi apply`
...
4. Create any utility scripts under `scripts/` if needed (use chezmoi `executable_` prefix for script filenames so they deploy as executable)
5. For personal chezmoi-managed skills: add the skill directory under `exact_skills/<name>/` in the chezmoi source, then run `chezmoi apply`
...
- [ ] Changes follow [references/skills-setup.md](references/skills-setup.md) (add to `exact_skills/<name>/`, `chezmoi apply`)
